...
def load_model(filename):
    file_path = os.path.join(BASE_DIR, filename)  # Ensure correct path
    try:
        return pickle.load(open(file_path, "rb"))
    except FileNotFoundError:
        st.error(f"Error: The model file '{filename}' is missing. Ensure it exists in the GitHub repository and Streamlit Cloud.")
        return None

# Load models (since they are in the same folder as app.py)
